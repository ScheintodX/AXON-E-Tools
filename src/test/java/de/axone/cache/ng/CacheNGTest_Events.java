...
public class CacheNGTest_Events {
	...
	public void listenersReceiveEvents() throws Exception {
		...
		CacheNG.AutomaticClient<Aid, TArticle> autoMaster =
				spy( backend.automatic( RN.TOP_LARTICLE.unique(), accMaster ) );
		
		CacheNG.AutomaticClient<Aid, TArticle> autoSlave =
				spy( backend.automatic( RN.TOP_LARTICLE.unique(), accSlave ) );
		
		TArticle art;
		
		art = autoMaster.fetch( A12345 );
		assertNotNull( art );
		assertThat( autoMaster ).hasCached( A12345 );
		verify( accMaster ).get( A12345 );
		
		art = autoSlave.fetch( A12345 );
		assertNotNull( art );
		assertThat( autoMaster ).hasCached( A12345 );
		verify( accMaster ).get( A12345 );
		...
		art = autoSlave.fetch( A12345 );
		verify( accMaster, times( 2 ) ).get( A12345 );
